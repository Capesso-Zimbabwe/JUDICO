...
class Command(BaseCommand):
    help = 'Creates 11 sample tasks for testing'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Starting to create sample tasks...'))
        
        # Get all clients
        clients = list(Client.objects.all())
        if not clients:
            self.stdout.write(self.style.ERROR('No clients found in the database. Please create some clients first.'))
            return
        
        # Get all lawyers (users with lawyer_profile)
        lawyers = list(User.objects.filter(lawyer_profile__isnull=False))
        if not lawyers:
            self.stdout.write(self.style.ERROR('No lawyers found in the database. Please create some lawyers first.'))
            return
        
        # Get a staff user for created_by
        staff_users = list(User.objects.filter(is_staff=True))
        if not staff_users:
            self.stdout.write(self.style.ERROR('No staff users found in the database. Please create a staff user first.'))
            return
